...
from ..database import get_ch, BatchWriter
...
class MqttConsumer:
    ...
    def __init__(self):
        ...
        self.batch_writer = BatchWriter(
            self.ch,
            batch_size=settings.mqtt_batch_size,
            flush_interval=30.0,
        )
        ...
        self._drain_size = settings.mqtt_batch_size
    ...
    def _batch_writer_loop(self):
        ...
        while not self._stop_event.is_set():
            try:
                env_batch = self._drain_queue(self.env_queue, self._drain_size)
                if env_batch:
                    rows, cols = self._prepare_env_rows(env_batch)
                    self.batch_writer.add("env_sensor_data", rows, cols)
                ph_batch = self._drain_queue(self.ph_queue, self._drain_size)
                if ph_batch:
                    rows, cols = self._prepare_ph_rows(ph_batch)
                    self.batch_writer.add("ph_sensor_data", rows, cols)
            except Exception as e:
    ...
    def _prepare_env_rows(self, batch: List[Dict]):
        columns = [
    ...
    def _prepare_ph_rows(self, batch: List[Dict]):
        columns = [
            "timestamp", "sensor_id", "shelf_id", "slot_id",
    ...
    def stop(self):
        ...
        if remaining_env:
            rows, cols = self._prepare_env_rows(remaining_env)
            self.batch_writer.add("env_sensor_data", rows, cols)
        if remaining_ph:
            rows, cols = self._prepare_ph_rows(remaining_ph)
            self.batch_writer.add("ph_sensor_data", rows, cols)
